...
<html lang="en" dir="ltr">
  ...
  <body>
    <h3 class="text-center">Laporan Peminjaman Ruangan {{$periode ? "Periode ".$periode : "Semua Periode"}}</h3>
    <br><br>
    @if ($ruangan)
      <table>
        <tr>
      ...
      <tbody>
        ...
        @foreach ($transaksis as $key => $value)
          <tr>
            ...
            <td class="text-center">{{ $value->peminjam->user->name}}</td>
            <td class="text-center">{{ $value->peminjam->nama_organisasi}}</td>
            <td class="text-center">{{ $value->nama_acara}}</td>
            <td class="text-center">{{ $value->tanggal_mulai->format('d/m/Y')}}</td>
            <td class="text-center">{{ $value->tanggal_selesai->format('d/m/Y')}}</td>
